  ...
  const deleteOrderHandler = async (orderId) => {
    try {
      const response = await axios.delete(
        `${backend_url}/api/order/delete/${orderId}`,
        {
          headers: { token },
        }
      );
    ...
    <div className="px-2 sm:px-8 mt-4 sm:mt-14">
      <div className="flex flex-col gap-4">
        {orders.map((order) => (
          <div
            key={order._id}
            className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-[0.5fr_2fr_1fr_0.5fr_1fr_1fr] gap-4 items-start p-3 text-gray-700 bg-white rounded-lg"
          >
            ...
            <div className="flex flex-col gap-2">
              ...
              {/* Delete Button - Only Visible When Delivered */}
              {order.status === "Delivered" && (
                <button
                  onClick={() => deleteOrderHandler(order._id)}
                  className="mt-2 bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 transition text-xs font-semibold"
                >
                  Delete Order
                </button>
              )}
